...
Running Svenager in production. Development setup lives in the README;
...
1. `git pull`, then `docker compose -f docker-compose.prod.yml up -d
   --build`. Flyway applies pending schema migrations on startup; the
   server refuses to start on a failed migration rather than running on a
   half-migrated schema.
2. Agent fleet: see "Agent updates" below.
